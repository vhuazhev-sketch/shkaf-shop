...
// Данный файл загружается как обычный скрипт (не модуль), поскольку
// GitHub Pages и некоторые мобильные браузеры (в том числе Telegram
// WebApp) имеют проблемы с ES‑модулями.  Вместо использования
// статических импортов мы берём необходимые функции из Firebase
// compat SDK, который подключён в index.html.  Это обеспечивает
// совместимость и корректную работу на всех платформах.

// Получаем ссылки на глобальные сервисы, которые были инициализированы
// в index.html.  Эти переменные определены через window.db,
// window.auth и т.д. и доступны по всему приложению.
const db = window.db;
const auth = window.auth;
const storage = window.storage;

// Обёртки для Firestore, имитирующие API модульной версии.  Они
// позволяют использовать те же имена функций (collection, doc и т.д.)
// что и в модульной версии, но под капотом вызывают compat API.
const collection = (dbParam, name) => {
  return dbParam.collection(name);
};
const doc = (dbParam, collectionName, id) => {
  return dbParam.collection(collectionName).doc(id);
};
...
const getDocs = async (queryOrRef) => queryOrRef.get();
const onSnapshot = (queryOrRef, callback) => queryOrRef.onSnapshot(callback);
const where = (field, op, value) => ({ field, op, value });
const query = (colRef, whereClause) => colRef.where(whereClause.field, whereClause.op, whereClause.value);
const increment = (num) => firebase.firestore.FieldValue.increment(num);

// Обёртки для аутентификации.  Они повторяют сигнатуру функций
// модульной версии, но используют compat API.  Мы передаём auth
// как первый параметр для совместимости с существующим кодом.
const signInWithEmailAndPassword = (authObj, email, password) => authObj.signInWithEmailAndPassword(email, password);
const signOut = (authObj) => authObj.signOut();
const onAuthStateChanged = (authObj, callback) => authObj.onAuthStateChanged(callback);
